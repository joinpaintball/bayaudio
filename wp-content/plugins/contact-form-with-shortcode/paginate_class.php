...
class afo_paginate{
	...
	function __construct($per_page,$base_url){
		$this->per_page	= $per_page;
		$this->base_url	= $base_url;
	}
	
	function afo_paginate_css(){ ?>
	<style>
	.page_list_cont{
		clear:both;
		margin-top:20px;
	...
	function initialize($query,$current_page = ''){
		global $wpdb;
		if(!$query){
			return;
		}
	...
	function gen_query_str($query_str = ''){
