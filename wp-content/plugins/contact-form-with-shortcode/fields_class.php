...
class fields_class {
	...
	function contact_afo_scripts_fr(){
	?>
	...
	function genField($field = 'text',$name = '',$id='',$value = '', $desc = '', $options, $required=''){
		switch ($field){
			case 'text':
				...
				echo '<input type="text" name="'.$name.'" id="'.$id.'" placeholder="'.$valueZ.'" value="'.$value.'" '.$required.'>';
				echo $this->get_field_desc($desc);
			break;
			case 'textarea':
				echo '<textarea name="'.$name.'" id="'.$id.'" '.$required.' placeholder="Message">'.$value.'</textarea>';
				echo $this->get_field_desc($desc);
			break;
			case 'select':
